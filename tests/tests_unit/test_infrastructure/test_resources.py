...
class DummyEngine:
    ...
    def dispose(self):
        """
        Make dispose both sync-callable and awaitable-compatible.
        - If someone calls engine.dispose() synchronously -> flag sets True, fine.
        - If someone does `await engine.dispose()` -> Python awaits the returned object,
          which immediately completes; flag was already set to True.
        """
        self.disposed = True
        ...
        class _AwaitableNoop:
            def __await__(self):
                if False:
                    yield  # required to make this a generator-based awaitable
                return None
